angular.module('entourageApp', ['ui.bootstrap', 'ImageCropper', 'ngTouch'])
  .controller('MapController', ['$scope', '$filter', '$http', '$uibModal', '$q', function($scope, $filter, $http, $uibModal, $q) {
    ...
    map.initMap = function(position) {
      ...
      google.maps.event.addListener(map.mapObject, 'bounds_changed', function() {
        if (!map.isMapReady) {
          ...
          if (map.public) {
            map.getPublicActions()
            .then(map.getPois(true))
            .then(function(){
              map.getCurrentAddress();
      ...
      google.maps.event.addListener(map.mapObject, 'center_changed', map.refreshMap);
    ...
    map.getCurrentAddress = function() {
    ...
    map.refreshMap = debounce(function() {
      if (map.refreshing) {
        return;
      }
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (var i = 0; i < Object.keys(data).length; i++) {
            ...
            map.actions.push(map.associateMarker(action));
          }
    ...
    map.getPrivateFeed = function(first) {
      ...
      map.boundsSize = map.getBoundsSize(map.mapObject.getBounds());
      ...
      map.getApiFeed(data, feeds)
      .then(function() {

        feeds = feeds.map(function(action) {
        ...
        var actions = feeds.filter(function(action) {
          ...
          return map.associateMarker(action);
        });

        map.clearAllMarkers('actions');
        ...
        var announcements = feeds.filter(function(action) {
    ...
    map.getApiFeed = function(data, feeds, deferred) {
      if (!deferred) {
        var deferred = $q.defer();
      }

      data.before = feeds.length ? feeds[feeds.length - 1].data.updated_at : new Date();
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.feeds && response.feeds.length) {
            response.feeds.map(function(action) {
              feeds.push(action);
            });
            ...
            if (response.feeds[response.feeds.length - 1].data.updated_at.indexOf('1970-01-01') > -1) {
              map.getApiFeed(data, feeds, deferred);
            } else {
              deferred.resolve();
            }
          } else {
        ...
        error: function(response) {
          if (response.responseJSON && response.responseJSON.message && response.responseJSON.message == 'unauthorized') {
            map.logout();
          }
          deferred.resolve();
        }
    ...
    map.filterActions = function() {
      ...
      map.actions = map.actions.filter(function(action) {
        ...
        if (map.public && visible && action.group_type != 'outing' && map.filters.period != '') {
    ...
    map.getPois = function(first) {
      var deferred = $q.defer();

      map.clearAllMarkers('pois');
    ...
    map.getBoundsSize = function(bounds) {
    ...
    map.associateMarker = function(action) {
      ...
      action.marker.addListener('mouseover', function() {
        ...
        map.showMarkerTitle(action, this);
      });
      action.marker.addListener('mouseout', map.hideMarkerTitle);
    ...
    map.askLocation = function() {
      navigator.geolocation.getCurrentPosition(map.initMap, map.initMap);
    }


    // ** FILTERS ** //

    map.initFilters = function() {
    ...
    .then(function() {
      ...
      if (getQueryParams('ville') && getQueryParams('ville') != 'undefined') {
        map.initMapFromAddress(getQueryParams('ville'));
        return;
      }
      if (map.loggedUser && map.loggedUser.address) {
        if (map.loggedUser.address.latitude) {
          ...
          map.initMapFromAddress(map.loggedUser.address.display_address);
          return;
        }
      }
      if (localStorage.getItem('address')) {
        map.initMapFromAddress(localStorage.getItem('address'));
        return;
      }
    ...
    map.initMapFromAddress = function(address) {
      var geocoder = new google.maps.Geocoder;
